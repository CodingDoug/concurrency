...
/**
 * An example of an invalid loader that is an inner class that is not declared
 * static.  It will crash with the following exception:
 *
 * java.lang.IllegalArgumentException: Object returned from onCreateLoader must not be a non-static inner member class: ExampleLoader{418dec78 id=0}
 */

public class ActivityInvalidNonStaticLoader extends FragmentActivity {

    private static final String LOG_TAG = ActivityInvalidNonStaticLoader.class.getSimpleName();
    ...
    private class StringResourceLoaderCallbacks implements LoaderCallbacks<ResultOrException<String, Exception>> {
        ...
        public Loader<ResultOrException<String, Exception>> onCreateLoader(final int id, final Bundle args) {
            return new InvalidStringResourceLoader(ActivityInvalidNonStaticLoader.this, pair.res);
        }
    ...
    private class InvalidStringResourceLoader extends ExecutorServiceLoader<String> {
        private final int resource;

        public InvalidStringResourceLoader(final Context context, final int resource) {
